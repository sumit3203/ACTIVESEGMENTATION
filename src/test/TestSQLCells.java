		...
		while (enums.hasMoreElements()) {
			...
			//returning the sum of the probability vectors in order to check if sum=1
	        	...
	        	String update="INSERT INTO class_list (session_id, image_name, class_label) "
	            		+ 				"VALUES  ( ?, ?, ?)";
	        	//inserting in image_list the image name the instance type, the cell class label and the vector id
	        	PreparedStatement ips=con.prepareStatement(update);
				ips.setInt(1, sessionID);
	        	ips.setString(2,all.get(c)); // image_name.png
	        	// ips.setString(3,all.get(c+1)); // instance type = train or test
			    ...
			    // update="INSERT INTO class_probabilities ( session_id, class_label, probability) VALUES ( ?, ?, ?)";
				...
				for(int i = 0; i < probv.length; i++) {
					...
					// vlps.setString(2, all.get(c + 1));
				    // vlps.setDouble(3, vec);
				    // vlps.executeUpdate();
			 ...
			 // for looping into the prob vectors
			    // for(int i = 0; i < probv.length; i++) {
			    // 	vlps.setString(1, classlabels.get(i));
				//     vlps.setDouble(2, probv[i]);
				//     vlps.executeUpdate();
			    // }
			    ips.clearParameters();
			    vps.clearParameters();
			    ...
			    // vlps.close();
			    vid++;
				// update="INSERT INTO class_list (session_id, image_name, cl_label) "
			    //     		+ 				"VALUES  (?, ?, ?)";	    	
				// 	PreparedStatement clps=con.prepareStatement(update);
				// 		clps.setInt(1, sessionID);
				// 		clps.setString(2, all.get(c));
				// 		clps.setString(3,all.get(c+2));
				// 		clps.executeUpdate();
