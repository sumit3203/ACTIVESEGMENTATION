		...
		while (enums.hasMoreElements()) {
			...
			//returning the sum of the probability vectors in order to check if sum=1
	        	...
	        	String update="INSERT INTO image_list (image_name, instance_type, cell_type,v_id) "
	            		+ 				"VALUES  ( ?, ?,?,?)";
	        	//inserting in image_list the image name the instance type, the cell type and the vector id
			    ...
			    //inserting into vector list the vector id and the value of each probability vector for the specific vector
			 ...
			 // for looping into the prob vectors
			    ...
			    c=c+3;
			    if(c==3) {
			    	//inserting into class list the class names
				    ...
				    fnps.close();
				    //inserting the feature values, the feature name id and the class id
						   ...
						   for (double[] array_1D: array_2D) {
						        ...
						        if(array_1D!=null) {
						        	for(double elem:array_1D) {
						        		...
						        		int k[]=findIndex(w,elem);
						        		//find the index of the element of the array in order to get the feature name
						        		//for this specific feature value(assuming that values and name attributes are 1-1 correspondance)
						        		fps.setInt(2,hm.get(nm[k[0]][k[1]][k[2]]));
						        		//sets the feature name id(depending on the feature name) 
							        	fps.setInt(3,cl);
						        		fps.executeUpdate();
